...
__global__ void det_calc_onDevice(double *matrix, double* results, int n)
{
    ...
    int r_idx = threadIdx.x+blockDim.x*threadIdx.y+blockDim.x*blockDim.y*threadIdx.z;
    ...
    for(int i=0; i<n; i++){
            ...
            if(threadIdx.x == i){
                    results[m_idx] *= matrix[mat+i*n + i];
                    continue;
            }
            for(int j = i+1; j< n; j++)
                        matrix[row + j] -= matrix[mat+i*n + j] * matrix[row + i] / matrix[mat+i*n + i];
